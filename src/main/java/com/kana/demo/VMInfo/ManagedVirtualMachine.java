...
public class ManagedVirtualMachine {
    ...
    public ManagedVirtualMachine(VirtualMachine virtualMachine)
            throws Exception {
        if (virtualMachine != null)
            this.vm = virtualMachine;
        else
            throw new Exception("Null Virtual Machine. Unable to manage it.");
    }

    public String getName() {
        //return vm.getConfig().getGuestFullName();
        return vm.getConfig().getName();
    ...
    public long getCPUUsage() throws Exception {
        if (VirtualMachinePowerState.poweredOn.equals(vm.getRuntime()
            ...
            return vm.getSummary().getQuickStats().getOverallCpuUsage()
                    .longValue()
                    / vm.getConfig().getCpuAllocation().getLimit();
        throw new Exception(
                "Invalid Virtual Machine power state. Unable to manage it.");
    }
    ...
    public void shutDownGuestOnVM() throws TaskInProgress, InvalidState,
            ToolsUnavailable, RuntimeFault, RemoteException {
        vm.shutdownGuest();
    }
